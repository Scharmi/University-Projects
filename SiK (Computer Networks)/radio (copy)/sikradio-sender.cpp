...
void check_arguments(variables_map vm) {
    //TODO sprawdzić czy coś jeszcze muszę sprawdzać
...
void send_rexmit(MulticastSender &multicastSender, SafeFifo &fifo, SafeSet& packets_to_rexmit, uint64_t rtime, uint64_t session_id) {
    while(true) {
...
void listen_for_ctrl(uint16_t ctrl_port, SafeFifo& fifo, SafeSet& packets_to_rexmit, variables_map vm) {
    ...
    while(true) {
        ...
        if(received.get_type() == CtrlPackage::Type::REXMIT) {
            ...
            std::cout << "LOOKUP from" << received.get_client_address() << "\n";
...
int main(int argc, const char *argv[])
{
    ...
    std::thread listen_for_ctrl_thread(listen_for_ctrl, vm["CTRL_PORT"].as<uint64_t>(), std::ref(fifo), std::ref(packets_to_rexmit), vm);
    std::thread send_rexmit_thread(send_rexmit, std::ref(sender), std::ref(fifo), std::ref(packets_to_rexmit), vm["RTIME"].as<uint64_t>(), session_id);
    while(read) {
